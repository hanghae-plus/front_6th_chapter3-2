...
### 1. 데이터 구조 확장
- [ ] RepeatInfo 타입에 weekdays 필드 추가 (WeekDay[] 타입)
...
### 2. UI 구현
- [ ] 반복 유형이 '주간'일 때만 요일 선택 UI 표시
- [ ] Material-UI ToggleButton 활용한 요일 선택 컴포넌트
- [ ] 선택된 요일 시각적 피드백 (색상, 아이콘)
- [ ] 요일 선택에 따른 반복 패턴 미리보기
- [ ] 요일 패턴 프리셋 선택 UI (평일, 주말 등)

### 3. 비즈니스 로직
- [ ] calculateRepeatingDates 함수에 요일 필터링 로직 추가
- [ ] 요일 선택 유효성 검사 구현
- [ ] 선택된 요일에 대한 날짜 계산 로직
- [ ] 기존 반복 로직과의 통합
